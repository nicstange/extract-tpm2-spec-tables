...
impl<'a, B: Ord> PartialOrd for LowerBoundQuery<'a, B> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self.lb {
            IntervalBound::Inclusive(lhs_lb) => match &other.lb {
                IntervalBound::Inclusive(rhs_lb) => lhs_lb.partial_cmp(rhs_lb),
            ...
            IntervalBound::Exclusive(lhs_lb) => match &other.lb {
                ...
                IntervalBound::Exclusive(rhs_lb) => lhs_lb.partial_cmp(rhs_lb),
            },
        }
    }
}
...
impl<'a, B: Ord> PartialOrd for UpperBoundQuery<'a, B> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self.ub {
            IntervalBound::Inclusive(lhs_ub) => match &other.ub {
                IntervalBound::Inclusive(rhs_ub) => lhs_ub.partial_cmp(rhs_ub),
            ...
            IntervalBound::Exclusive(lhs_ub) => match &other.ub {
                ...
                IntervalBound::Exclusive(rhs_ub) => lhs_ub.partial_cmp(rhs_ub),
            },
        }
    }
}
...
impl<'a, B: Ord> PartialOrd<UpperBoundQuery<'a, B>> for LowerBoundQuery<'a, B> {
    fn partial_cmp(&self, other: &UpperBoundQuery<'a, B>) -> Option<Ordering> {
        match self.lb {
            IntervalBound::Inclusive(lhs_lb) => match &other.ub {
                IntervalBound::Inclusive(rhs_ub) => {
                    if *lhs_lb == *rhs_ub {
                        Some(Ordering::Less)
                    } else {
            ...
            IntervalBound::Exclusive(lhs_lb) => match &other.ub {
                IntervalBound::Inclusive(rhs_ub) => {
                    if *lhs_lb == *rhs_ub {
                        Some(Ordering::Greater)
                    } else {
...
impl<'a, B: Ord> PartialOrd<LowerBoundQuery<'a, B>> for UpperBoundQuery<'a, B> {
    fn partial_cmp(&self, other: &LowerBoundQuery<'a, B>) -> Option<Ordering> {
        other.partial_cmp(self).map(|o| match o {
...
impl<B: Clone + Ord + 'static, T> IntervalTree<B, T> {
    ...
    fn update_upper_bound_aux(
        subtree_ub: &mut UpperBoundAux<B>,
        key: &LowerBoundKey<B>,
    ...
    pub fn delete<IsMatch>(
        &mut self,
        key: &Interval<B>,
        is_match: &IsMatch,
    ) -> Option<(Interval<B>, T)>
    where
        for<'a> IsMatch: Fn(&'a Interval<B>, &'a T) -> bool,
    {
        let is_empty = LowerBoundQuery::new(&key.lb) > UpperBoundQuery::new(&key.ub);
        self.tree
            .delete(
                &LowerBoundQuery::new(&key.lb),
                &|node_key, node_value| {
...
pub struct IntervalTreeIterator<'a, B: Clone + Ord, T> {
    lb: Option<IntervalBound<B>>,
    it: AugmentedAVLTreeIterator<
        'a,
        IteratorLowerBoundQuery,
        IteratorUpperBoundQuery<B>,
        LowerBoundKey<B>,
        T,
        UpperBoundAux<B>,
...
impl<'a, B: Clone + Ord + 'static, T> IntervalTreeIterator<'a, B, T> {
    fn new(
        ...
        Self {
            ...
            it: tree.tree.iter(
                None,
                ub.map(|ub| IteratorUpperBoundQuery::new(ub)),
        ...
        loop {
            ...
            if let Some(query_lb) = &self.lb {
                if let Some(item) = &item {
                    if !matches!(
                        LowerBoundQuery::new(query_lb)
                            .partial_cmp(&UpperBoundQuery::new(&item.0.interval.ub))
                            .unwrap(),
                        Ordering::Less
                    ) {
                        continue;
...
pub struct IntervalTreeMutIterator<'a, B: Clone + Ord, T> {
    lb: Option<IntervalBound<B>>,
    it: AugmentedAVLTreeMutIterator<
        'a,
        IteratorLowerBoundQuery,
        IteratorUpperBoundQuery<B>,
        LowerBoundKey<B>,
...
fn test_insert_delete_same() {
    ...
    t.insert(
        Interval::new(IntervalBound::Inclusive(0), IntervalBound::Inclusive(1)),
        0,
    );
    t.insert(
        Interval::new(IntervalBound::Inclusive(0), IntervalBound::Inclusive(1)),
    ...
    assert_eq!(
        t.iter(
            Some(IntervalBound::Inclusive(0)),
            Some(IntervalBound::Inclusive(1))
        )
        .count(),
        3
    );
    assert_eq!(
        t.iter(
            Some(IntervalBound::Exclusive(0)),
            Some(IntervalBound::Inclusive(1))
        )
        .count(),
        3
    ...
    assert_eq!(
        t.iter(
            Some(IntervalBound::Inclusive(0)),
            Some(IntervalBound::Exclusive(1))
        )
        .count(),
        3
    );
...
fn test_insert_delete_many_inclusive() {
    ...
    for i in 0..256 {
        t.insert(
            Interval::new(IntervalBound::Inclusive(i), IntervalBound::Inclusive(i + 2)),
            i,
        );
        assert_eq!(
            t.iter(None::<IntervalBound::<u32>>, None).count(),
            i as usize + 1
        );
        assert_eq!(
            t.iter_mut(None::<IntervalBound::<u32>>, None).count(),
            i as usize + 1
        );
    }
    ...
    for i in 0..255 {
        assert_eq!(
            t.iter(
                None::<IntervalBound::<u32>>,
                Some(IntervalBound::Inclusive(i + 1))
            )
            .count(),
            i as usize + 2
        );
        assert_eq!(
            t.iter(
                None::<IntervalBound::<u32>>,
                Some(IntervalBound::Exclusive(i + 1))
            )
            .count(),
            i as usize + 1
        );
    ...
    for i in 0..255 {
        assert_eq!(
            t.iter(
                Some(IntervalBound::Inclusive(256 - i)),
                None::<IntervalBound::<u32>>
            )
            .count(),
            i as usize + 2
        ...
        assert_eq!(
            t.iter(
                Some(IntervalBound::Exclusive(256 - i)),
                None::<IntervalBound::<u32>>
            )
            .count(),
            i as usize + 1
    ...
    for i in 2..254 {
        assert_eq!(
            t.iter(
                Some(IntervalBound::Inclusive(i)),
                Some(IntervalBound::Inclusive(i + 2))
            )
            .count(),
            5
        );
        assert_eq!(
            t.iter(
                Some(IntervalBound::Exclusive(i)),
                Some(IntervalBound::Inclusive(i + 2))
            )
            .count(),
            4
        ...
        assert_eq!(
            t.iter(
                Some(IntervalBound::Inclusive(i)),
                Some(IntervalBound::Exclusive(i + 2))
            )
            .count(),
            4
        );
...
fn test_insert_delete_many_excusive() {
    ...
    for i in 0..256 {
        t.insert(
            Interval::new(IntervalBound::Exclusive(i), IntervalBound::Exclusive(i + 2)),
            i,
        );
        assert_eq!(
            t.iter(None::<IntervalBound::<u32>>, None).count(),
